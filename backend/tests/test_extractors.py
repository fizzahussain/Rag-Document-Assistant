...
def test_html_extractor():
    ...
    content = (
        ...
        b"<body><h1>Header</h1>"
        b"<p>Paragraph text</p></body></html>"
    )
...
def test_pdf_extractor_text():
    ...
    page.insert_text(
        (50, 50),
        "Sample PDF Text Content For Testing Purpose",
    )
    pdf_bytes = doc.write()
    doc.close()
...
def test_extractor_factory():
    assert isinstance(
        ExtractorFactory.get_extractor("doc.pdf"),
        PDFExtractor,
    )
    assert isinstance(
        ExtractorFactory.get_extractor("file.txt"),
        TextExtractor,
    )
    assert isinstance(
        ExtractorFactory.get_extractor("data.json"),
        JSONExtractor,
    )
