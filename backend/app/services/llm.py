...
class MockLLMProvider(BaseLLMProvider):
    ...
    async def generate_answer(
        ...
        top_ref = (
            f"[{top_source.filename}, page {top_source.page_number}]"
            if top_source.page_number
            else f"[{top_source.filename}, chunk {top_source.chunk_index}]"
        )
        ...
        answer_text = (
            ...
            f"Sources reviewed:\n"
            + "\n".join(
                f"- {c.filename} ({'page ' + str(c.page_number) if c.page_number else 'chunk ' + str(c.chunk_index)})"
                for c in citations
            )
        )
    ...
    async def generate_answer(
        ...
        for idx, src in enumerate(sources):
            ...
            formatted_context.append(
                f"SOURCE [{src.filename}, {page_str}]:\n{src.text}\n"
            )
            citations.append(
                Citation(
                    filename=src.filename,
